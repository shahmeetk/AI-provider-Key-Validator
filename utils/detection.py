...
class ProviderDetector:
    ...
    OPENAI_PROJ_REGEX = re.compile(r'sk-proj-\w{120,}')
    ...
    def detect_provider(cls, api_key: str) -> Optional[APIKey]:
        ...
        if api_key.startswith("sk-ant-"):
            ...
            # This is the new OpenAI project key format
            return create_api_key(Provider.OPENAI, api_key)
        elif api_key.startswith("sk-"):
            # Could be OpenAI, Deepseek, or Anthropic
            if cls.OPENAI_REGEX.match(api_key) and len(api_key) >= 51:
                # OpenAI keys are typically 51 characters long (including the "sk-" prefix)
                return create_api_key(Provider.OPENAI, api_key)
